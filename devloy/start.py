...
class StartCommand:
    ...
    def __init__(self, logger, all_deps):
        self.logger = logger
    ...
    def find_dep_dir(self, repository, worktree):
        # TODO in default conf file
        repository_search_paths = ['/home/ricardo/workspace/eprosima']

        for search_path in repository_search_paths:
...
def start_verb_init(args, logger):
    """
    ...
    * Find projects information:
    ...
    """
    arguments = vars(args)
    command = StartCommand(logger, arguments['all_deps'])

    command.get_projects_info()
